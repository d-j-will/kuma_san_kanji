defmodule KumaSanKanji.Seeds.ContentSeeder do
  ...
  def seed_kanji_thematic_groups do
    ...
    results_with_subgroups =
      Enum.flat_map(mappings_with_subgroups, fn %{
                                                  group: group_name,
                                                  subgroup: subgroup,
    ...
    results_without_subgroups =
      Enum.flat_map(mappings_without_subgroups, fn %{group: group_name, kanji_chars: kanji_chars} ->
  ...
  def seed_educational_contexts do
    ...
    Enum.each(contexts, fn context ->
      case Content.create_educational_context(context) do
        ...
        {:error, error} ->
          IO.puts(
            "Failed to create educational context for Grade #{context.grade_level}: #{inspect(error)}"
          )
      end
    end)
  end
  ...
  def seed_kanji_learning_meta do
    learning_meta_data = [
      ...
      {"木",
       [
         ...
         """,
         mnemonic_hints:
           "Picture a tree with branches at the top, a trunk in the middle, and roots at the bottom"
       ]}
    ]
    ...
    Enum.each(learning_meta_data, fn {character, meta_items} ->
      with {:ok, [kanji]} <-
             RootDomain.read_kanji(KumaSanKanji.Kanji.Kanji, filter: [character: character]) do
      ...
      kanji_chars_processed =
        Enum.map(kanji_chars, fn char ->
          ...
          case RootDomain.read_kanji(Kanji, filter: [character: char]) do
            ...
            _ ->
              {:error, "Kanji #{char} not found"}
          end
        end)
        # Remove nils if kanji not found
