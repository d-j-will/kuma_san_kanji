defmodule KumaSanKanjiWeb.KanjiStrokeOrderComponent do
  ...
  def stroke_order(assigns) do
    ...
    ~H"""
    <div id={@id} class={["stroke-order-container", @style_mode, @class, "flex flex-col items-center justify-center text-center"]}
         data-kanji={@kanji}
         ...
         role="img"
         aria-label={if @mode == :trace, do: "Canvas for tracing kanji strokes", else: "Stroke order diagram for kanji #{@kanji}"}>
      <%= if @svg_markup do %>
        <div class="relative">
          <%= raw(@svg_markup) %>
        ...
        <div class="mt-2 flex gap-2 items-center justify-center text-xs" id={"controls-#{@kanji}"} phx-hook="AudioFeedback">
          ...
          <button type="button" class="px-2 py-1 rounded bg-wabi-stone/20 hover:bg-wabi-stone/30"
                  phx-click="stroke_order_step" phx-value-kanji={@kanji} phx-value-mode={@style_mode} aria-label="Step through strokes">
            Step
          </button>
          <button type="button" class="px-2 py-1 rounded bg-wabi-stone/20 hover:bg-wabi-stone/30"
                  phx-click="stroke_order_toggle_style" phx-value-kanji={@kanji} aria-label="Toggle stroke style">
            Toggle Style
          </button>
        </div>
      <% else %>
        <div class="p-4 text-sm text-wabi-stone/70" aria-live="polite">Stroke order SVG not available.</div>
      <% end %>
    </div>
    """
  end
    ...
    if String.length(kanji) == 1 do
      ...
      contents =
        case KumaSanKanji.KanjiVG.Cache.get(cache_key) do
          ...
          :miss ->
            path = Path.join([Application.app_dir(:kuma_san_kanji), "priv", "static", "kanjivg", padded <> ".svg"])
