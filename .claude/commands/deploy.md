Run pre-deploy verification and tag a new version for deployment.

## Steps

1. Run quality checks: `mix compile --warnings-as-errors && mix format --check-formatted && mix test && mix hex.audit && mix deps.unlock --check-unused`
...
4. Determine the next version by incrementing the patch number (e.g., v0.1.0 -> v0.1.1). If the user provided a specific version, use that instead.
